...
(1) 3个PGM模块
    ...
    (各自包含1组PC参数、1个linear和1个3x3 conv)

(2) 3个PIM模块
    self.noise_level3, self.reduce_noise_level3, 
    self.noise_level2, self.reduce_noise_level2, 
    self.noise_level1, self.reduce_noise_level1

    (各自包含1个Transformer块和1个1x1 conv)
...
class PromptIR(nn.Module):
    def __init__(self, 
        ...
        out_channels=3, 
        dim = 48,
        num_blocks = [4,6,6,8],   # 可以调整Transformer Blocks数量
        num_refinement_blocks = 4,
        heads = [1,2,4,8],
        ffn_expansion_factor = 2.66,
        bias = False,
        LayerNorm_type = 'WithBias',   ## Other option 'BiasFree'
        decoder = False,  # True启用PromptIR模块
    ):
        ...
        if self.decoder:
            ...
            # self.prompt3 = PromptGenBlock(prompt_dim=256,prompt_len=5,prompt_size = 16,lin_dim = 384)
        ...
        # self.noise_level3 = TransformerBlock(dim=256+384, num_heads=heads[2], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type)
        # self.reduce_noise_level3 = nn.Conv2d(256+384,int(dim*2**2),kernel_size=1,bias=bias)
    ...
    def forward(self, inp_img,noise_emb = None):
        ...
        latent = self.latent(inp_enc_level4)  # Fl

        if self.decoder:
            # PGM
            dec3_param = self.prompt3(latent)  # P
            # PIM
            latent = torch.cat([latent, dec3_param], 1)
            latent = self.noise_level3(latent)  # Transformer
            latent = self.reduce_noise_level3(latent)  # 1x1 conv 
    ...
    def initialize_weights(self, m):
        if isinstance(m, nn.Linear):
            ...
            if m.bias is not None:
                init.constant_(m.bias, 0.1)  # 偏置初始化
        elif isinstance(m, nn.Conv2d):
            init.kaiming_normal_(m.weight, nonlinearity='relu')  # Kaiming 初始化
            if m.bias is not None:
